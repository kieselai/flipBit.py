...
def lstToStr(lst, printBrackets=False, isFirst=True):
    returnStr = ""
    printBracketsAndNotFirst = printBrackets and not isFirst
    if isinstance(lst, list):
		...
		quoted = "'" + lst[i] + "'"
                returnStr += quoted if printBracketsAndNotFirst else lst[i]
	    elif isinstance(lst[i], int): 
                   returnStr += str(lst[i])
            if i!= len(lst)-1 and printBracketsAndNotFirst:
                returnStr+=","
	if printBracketsAndNotFirst:
            returnStr = "[" + returnStr + "]"
...
fileOut = fileName if args.fileOut == "" else args.fileOut
